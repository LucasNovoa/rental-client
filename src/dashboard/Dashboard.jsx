...
const Dashboard = () => {
  ...
  const typePerson = users?.map((user) => user.typePerson)
  const rolePerson = users?.map((user) => user.role)
  ...
  const data = {
    datasets: [
      {
        label: 'Usuarios - Tipo de Persona y Roles',
        data: countOccurrences(arrayData),
        borderWidth: 2,
  ...
  return (
    <>
      ...
      <div className='max-w-7xl mx-auto py-6 sm:px-6 lg:px-8'>
        <DChart className='' chartData={data} />
      </div>
      <div className='flex flex-col max-w-7xl mx-auto py-6 sm:px-6 lg:px-8'>
                    ...
                    <tr key={person?.email}>
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <div className='flex items-center'>
                          ...
                          <div className='ml-4'>
                            <div className='text-sm font-medium text-gray-900'>{person?.organization || (person?.firstName + ' ' + person?.lastName)}</div>
